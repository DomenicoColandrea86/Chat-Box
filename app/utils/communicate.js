...
module.exports = function(server) {
  ...
  // set socket.io log level
  io.set('log level', 2);

  // revert to 'xhr-polling' if client doesn't support websockets
...
socket.on('send', function(data) {
  if (typeof people[socket.id] === 'undefined') {
    ...
    if (io.sockets.manager.roomClients[socket.id]['/'+socket.room]) {
      ...
      // save to mongodb
      var newMsg = new Chat({room: socket.room, message: data.message, name: data.name});
      newMsg.save(function(err){
        // if error throw error else send message
        if (err) throw err;
    ...
    if (!exists) {
      ...
      if (!flag) {
        ...
        if (roomName.length !== 0) {
              ...
              // fetch docs in db whose room matches the room joined
              var query = Chat.find({room: socket.room});
              // sort in desc order by dy created ; limit number to show to 10
              query.sort('-created').limit(10).exec(function (err, docs) {
                // handle error
                if (err) throw err;
                // log sending chat history
                console.log('sending chat history');
                // reverse order of docs since we're grabbing them in descending order from mongodb to display the last 10 saved
                docs.reverse();
                // send to client
                utils.sendToSelf(socket, 'sendChatMessageHistory', docs);
              });
            }
          }
        }
