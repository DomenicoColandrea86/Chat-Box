...
var appControllers = appControllers || angular.module('appControllers', ['appServices', 'angularMoment']);

appControllers.controller('ChatAppCtrl', function ChatAppCtrl($scope, $q, $modal, socket, useragent, geolocation) {
  $scope.peopleCount = 0;
  $scope.messages = [];
  ...
  socket.on('sendChatMessageHistory', function(data) {
    angular.forEach(data, function(messages) {
...
appControllers.controller('VideoCtrl', function VideoCtrl($scope, socket) {
